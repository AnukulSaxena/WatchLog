...
const Header = () => {
  ...
  return (
    <>
      {isSearchBarOpen && (
        <div className="z-10 will-change-scroll fixed flex pt-40 md:px-40 justify-center inset-0 bg-neutral800 backdrop-blur-sm bg-opacity-30">
          <SeachBar toggleSearchBar={toggleSearchBar} />
        </div>
      )}

      <header
        ref={filterPanelRef}
        className={`fixed top-0 z-50 w-full  border-gray-200 px-2 lg:px-6 py-4 bg-neutral-800 duration-300 transition-transform ${
            ...
            <ul className="flex flex-col mt-4 w-full font-medium lg:flex-row lg:space-x-8 lg:mt-0">
              {navItems.map((item) =>
                item.active ? (
                  <li
                    key={item.name}
                    className={`${isMenuOpen ? " w-full" : ""} text-center`}
                  >
                    <button
                      value={item.slug}
                      ...
                      className={`block ${
                        isMenuOpen ? " w-full" : ""
                      } py-2 pr-4 pl-3  border-b    lg:border-0 lg:hover:text-primary-700 lg:p-0 text-gray-400 lg:hover:text-white hover:bg-gray-700 hover:text-white lg:hover:bg-transparent border-gray-700`}
                    >
                      {item.name}
                    </button>
                  </li>
              ...
              {authStatus && <LogoutBtn isMenuOpen={isMenuOpen} />}
            </ul>
          </div>
        </div>
      </header>
