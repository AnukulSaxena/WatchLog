...
function Avatar({ item }) {
    ...
    return (
        <div key={item.id} className="text-center min-w-20 max-w-20 md:min-w-32  text-white">
                ...
                className="md:w-32 min-h-32 md:min-h-48 hover:cursor-pointer overflow-hidden mb-3">
                {
                    !item?.profile_path ?
                        <div role="status" className=" rounded shadow animate-pulse md:p-6">
                            <div className="flex items-center justify-center h-32 md:mb-4 rounded bg-gray-700">
            ...
            <p className="truncate text-center mt-3 md:text-xl text-zinc-300">{item.name}</p>
            <div className="truncate text-center mt-1 px-2 md:px-5 text-md text-zinc-400">{item.character || item.job}</div>
        </div>
    )
}
