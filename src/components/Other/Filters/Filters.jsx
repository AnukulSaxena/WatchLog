...
function Filters() {
  ...
  function handleOptions() {
    switch (currentOption.name) {
      case "Genre":
        setOptions(
          mediaType === "movie"
            ? filterData?.genre?.movieGenre
            : filterData?.genre?.tvGenre
        );
        break;
      case "Language":
  ...
  function handlemultiOptionClick(e) {
    const selectedOption = multiOption.find(
  ...
  return (
    !loading && (
      <div
        className={`flex justify-center z-10 w-full h-9 px-4 lg:px-6 bg-neutral-700`}
      >
        <button
          id="resetButton"
          onClick={togglePanel}
        ...
        >
          ...
          >
              ...
              <div className="w-1/3 z-50 h-full border-r border-neutral-600">
                <ul>
                  {multiOption?.map((item, index) => (
                    <li
                      key={index}
                      className="w-full h-8 text-center text-white"
                    >
                      <button
                        value={item.name}
            ...
            <div className="w-full h-11 bg-neutral-700 border-t p-1 border-neutral-600 flex rounded-b justify-center text-white">
              <button
                onClick={handleReset}
