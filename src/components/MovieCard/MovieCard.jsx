...
const MovieCard = ({
  ...
  const { movieData, mode, movieDataIndex } = useSelector(
    (state) => state.movie
  );
  ...
  const deleteMovieDocInExpress = async () => {
    ...
    setIsChecked(false);
    const isDeleted = await movieServicex.removeId(
      data.id,
      mediaType,
      playlistId
    );
    if (!isDeleted) setIsChecked(true);
  };
  ...
  const handleCheckboxToggle = (e) => {
    ...
    if (status) {
      if (isChecked) {
        mode
          ? deleteMovieDocInExpress()
          : window.alert("Please Change the Mode");
      } else {
  ...
  useEffect(() => {
    ...
    if (crossCheck && movieData.length) {
      ...
      const foundObject = movieData[movieDataIndex][`${mediaType}Id`]?.find(
  ...
  return (
    <div className=" w-44 flex items-center relative justify-center md:w-48 md:h-72 h-64 rounded-md overflow-hidden">
      <MovieCardSkeleton className={`absolute inset-0`} />
      <div
      ...
      >
        ...
        >
          <button
            className={`absolute bottom-1 left-1  inline-flex flex-shrink-0 h-6 w-11 border-2 border-transparent rounded-lg  transition-colors ease-in-out duration-200 focus:outline-none ${
              isChecked ? "bg-neutral-500" : "bg-gray-200"
            }`}
          >
            <span
              className={`inline-block h-5 w-5 rounded-lg bg-neutral-800 shadow transform ring-0 transition-transform ease-in-out duration-200 ${
                isChecked ? "translate-x-5" : "translate-x-0"
              }`}
            ></span>
          </button>
        </div>
        <DropdownMenu id={data.id} mediaType={mediaType} />
        <Rating
